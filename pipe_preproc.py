...
for out_path in OUT_PATHS :
    if not path.isdir(out_path[:-2]):
        mkdir(out_path[:-2])
        print('not existing folder, create folder : ', out_path[:-2])

for path,opath in zip(PATHS,OUT_PATHS):
    get_files = ip.get_pure_imagef(path = path,formats=['jpg','.JPG'])
...
#         print("array of objects :  " ,lab_arr)
        idx_crop = ip.find_rect2(np.array(img_bicr),margin=OUT_MARGIN)    # Get index rectangular position
        print(idx_crop)
